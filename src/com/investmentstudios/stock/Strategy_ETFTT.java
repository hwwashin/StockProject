...
//					System.out.println("the date is --> " + dates[k] + "  and the trend value is --> " + trend[k]);
	...
	public static String findPDExit(int signalloc, String parameterlist) {
		
//		System.out.println("signalloc --> " + signalloc);
		
		String[] parameter = parameterlist.split(",");
		int profittargetstrategy = Integer.parseInt(parameter[0]);
		double greenpercentrisk = Double.parseDouble(parameter[1]);
		double graypercentrisk = Double.parseDouble(parameter[2]);
		double redpercentrisk = Double.parseDouble(parameter[3]);
		int trailingstopstrategy = Integer.parseInt(parameter[4]);
 ...
 */
	...
	/* int candlesize, int bodysize, int lowerlows, int percentcloselevel, int below3mas, double divergencevalue1, 
	 * double divergencevalue2, int channellocation, double rrrvalue 
	 */
	public static boolean isPDBuyMain(int pos, String parameterstring) {

		String[] parameterlist = parameterstring.split(",");
		int candlesize = Integer.parseInt(parameterlist[0]);
		int bodysize = Integer.parseInt(parameterlist[1]);
		int lowerlows = Integer.parseInt(parameterlist[2]);
		int percentcloselevel = Integer.parseInt(parameterlist[3]);
		int below3mas = Integer.parseInt(parameterlist[4]);
		double divergencevalue1 = Double.parseDouble(parameterlist[5]);
		double divergencevalue2 = Double.parseDouble(parameterlist[6]);
		int channellocation = Integer.parseInt(parameterlist[7]);
		double rrrvalue = Double.parseDouble(parameterlist[8]);
		...
		String parameterlist2 = candlesize + "," + bodysize + "," + lowerlows + "," + percentcloselevel + "," + below3mas + "," + divergencevalue1 + "," + divergencevalue2 + "," + channellocation + "," + rrrvalue;
		if(isInsideDay(pos) && !isPDBuyMain(pos+1, parameterlist2))
			return false;
		divergence[pos] = calculateDivergence(pos);	
